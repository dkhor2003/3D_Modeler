...
void MyWindow::s_keyboardCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if ( key == GLFW_KEY_A || key == GLFW_KEY_ESCAPE ||                                                                        // Default operation
		 key == GLFW_KEY_F || key == GLFW_KEY_R || key == GLFW_KEY_P || key == GLFW_KEY_Z || key == GLFW_KEY_T ||              // Camera operation
		 key == GLFW_KEY_B || key == GLFW_KEY_SPACE || key == GLFW_KEY_N || key == GLFW_KEY_M || key == GLFW_KEY_ENTER ||      // Surface operation
		 key == GLFW_KEY_U || key == GLFW_KEY_TAB ||                                                                           // Utility operation
		 key == GLFW_KEY_UP || key == GLFW_KEY_DOWN || key == GLFW_KEY_LEFT || key == GLFW_KEY_RIGHT ||                        // Bezier Surface Depth Manipulation 
		 key == GLFW_KEY_1 || key == GLFW_KEY_2 ||                                                                             // Light position control  
		 key == GLFW_KEY_LEFT_CONTROL || key == GLFW_KEY_RIGHT_CONTROL || key == GLFW_KEY_S || key == GLFW_KEY_C)              // Model saving and checking                                                 
...
void MyWindow::pollEvents()
{
	...
	// Pressing up and down each frame will result in continuous increase or decrease of point depth
	if (glfwGetKey(m_pWindow, GLFW_KEY_UP) == GLFW_PRESS) m_pMyApplication->depthControl(true);
	if (glfwGetKey(m_pWindow, GLFW_KEY_DOWN) == GLFW_PRESS) m_pMyApplication->depthControl(false);
	
	if (glfwGetKey(m_pWindow, GLFW_KEY_1) == GLFW_PRESS) // Move light in clockwise direction around the model
	{ 
		m_pMyApplication->updateLightPosition(true);
	}
    if (glfwGetKey(m_pWindow, GLFW_KEY_2) == GLFW_PRESS) // Move light in counterclockwise direction around the model
	{
		m_pMyApplication->updateLightPosition(false);
	}
...
void MyWindow::keyboardEvent(int key, bool bKeyDown)
{
	...
	else if (key == GLFW_KEY_ENTER && bKeyDown)
	{
		m_pMyApplication->removeOrMove(); 
	}
	else if (key == GLFW_KEY_U && bKeyDown)
	{
		m_pMyApplication->showUtilityButtons();
	}
	else if (key == GLFW_KEY_TAB && bKeyDown)
	{
		m_pMyApplication->switchModelingMode(); 
	}
	else if (key == GLFW_KEY_C && bKeyDown)
	{
		m_pMyApplication->checkModel(); 
	}
	else if ((key == GLFW_KEY_LEFT || key == GLFW_KEY_RIGHT) && bKeyDown)
	{
		bool next = (key == GLFW_KEY_RIGHT) ? true : false; 
		m_pMyApplication->nextPoint(next); 
	}
	else if (key == GLFW_KEY_LEFT_CONTROL || key == GLFW_KEY_RIGHT_CONTROL)
	{
		if (bKeyDown) m_bCTRL_down = true;
		else m_bCTRL_down = false; 
	}
	else if (key == GLFW_KEY_S)
	{
		if (bKeyDown) m_bS_down = true;
		else m_bS_down = false; 
	}
	if (m_bCTRL_down && m_bS_down)
	{
		m_pMyApplication->saveModel();  // save model if CTRL + S
	}
}
